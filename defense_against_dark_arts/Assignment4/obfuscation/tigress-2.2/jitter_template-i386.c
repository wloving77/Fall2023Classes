...
static struct output_buf *jit_internal_ob_new()
{
...
static void jit_internal_ob_free(struct output_buf *ob)
{
 free(ob->buf);
 free(ob);
}
static void jit_internal_ob_assert_space(struct output_buf *ob, int len)
{
...
static void jit_internal_ob_printf(struct output_buf *ob, const char *fmt, ...)
{
 char str[(8192)];
 va_list ap;
 va_start(ap, fmt);
 ...
 va_end(ap);
 len = strlen(str);
 jit_internal_ob_assert_space(ob, len);
 strcat(ob->buf, str);
 ob->size += len;
}
static void jit_internal_ob_append(struct output_buf *ob, char *str)
{
 int len = strlen(str);
 jit_internal_ob_assert_space(ob, len);
 strcat(ob->buf, str);
...
static void jit_internal_ob_pad(struct output_buf *ob, int size)
{
 while (strlen(ob->buf) < (size))
  jit_internal_ob_append(ob, " ");
}
...
static inline void jit_internal_print_addr(struct jit_disasm *disasm, struct output_buf *buf, jit_tree *labels, jit_op *op, int arg_pos)
{
 void *arg = (void *)op->arg[arg_pos];
 jit_tree *label_item = jit_tree_search(labels, (long) op);
 if (label_item) jit_internal_ob_printf(buf, disasm->label_forward_template, - (long) label_item->value);
 else {
  label_item = jit_tree_search(labels, (long) arg);
  if (label_item) jit_internal_ob_printf(buf, disasm->label_template, (long) label_item->value);
  else jit_internal_ob_printf(buf, disasm->generic_addr_template, arg);
 }
}
static inline void jit_internal_print_arg(struct jit_disasm *disasm, struct output_buf *buf, struct jit_op *op, int arg)
{
 long a = op->arg[arg - 1];
 if ((((op)->spec >> ((arg) - 1) * 2) & 0x03) == 0x02) jit_internal_ob_printf(buf, disasm->generic_value_template, a);
 if (((((op)->spec >> ((arg) - 1) * 2) & 0x03) == 0x01) || ((((op)->spec >> ((arg) - 1) * 2) & 0x03) == 0x03)) {
  char value[256];
  jit_get_reg_name(disasm, value, a);
  jit_internal_ob_append(buf, value);
 }
}
static inline void jit_internal_print_str(struct output_buf *buf, char *str)
{
 jit_internal_ob_append(buf, " \"");
 for (int i = 0; i < strlen(str); i++) {
  if (str[i] >= 32) {
   jit_internal_ob_printf(buf, "%c", str[i]);
  } else {
   char xbuf[16];
   switch (str[i]) {
   ...
   jit_internal_ob_append(buf, xbuf);
  }
 }
 jit_internal_ob_append(buf, "\"");
}
static void jit_internal_print_args(struct jit_disasm *disasm, struct output_buf *linebuf, jit_op *op, jit_tree *labels)
{
 for (int i = 1; i <= 3; i++) {
  if ((((op)->spec >> ((i) - 1) * 2) & 0x03) == 0x00) continue;
  jit_internal_ob_append(linebuf, i == 1 ? " " : ", ");
...
void jit_internal_print_full_op_name(struct output_buf *linebuf, jit_op *op)
{
 char *op_name = jit_get_op_name(op);
 jit_internal_ob_append(linebuf, op_name);
 if ((((jit_opcode) (op->code & 0xfff8)) == JIT_CALL) && ((op->code & 0x0007) & 0x02)) return;
 if ((op->code & 0x0007) & 0x02) jit_internal_ob_append(linebuf, "i");
 if ((op->code & 0x0007) & 0x01) jit_internal_ob_append(linebuf, "r");
 if ((op->code & 0x0007) & 0x04) jit_internal_ob_append(linebuf, "_u");
}
static int jit_internal_print_load_op(struct jit_disasm *disasm, struct output_buf *linebuf, jit_op *op)
{
 char rbuf[256];
 switch (((jit_opcode) (op->code & 0xfff8))) {
  case JIT_LREG:
   jit_internal_ob_append(linebuf, disasm->indent_template);
   jit_internal_ob_append(linebuf, jit_get_op_name(op));
   jit_internal_ob_pad(linebuf, 13);
   jit_get_reg_name(disasm, rbuf, op->arg[1]);
   jit_internal_ob_append(linebuf, rbuf);
   return 1;
  case JIT_UREG:
  case JIT_SYNCREG:
   jit_internal_ob_append(linebuf, disasm->indent_template);
  ...
  case JIT_RENAMEREG: {
    ...
    jit_internal_ob_append(linebuf, disasm->indent_template);
    jit_internal_ob_append(linebuf, jit_get_op_name(op));
    jit_internal_ob_append(linebuf, " ");
    jit_internal_ob_pad(linebuf, 13);
...
void jit_internal_print_comment(struct output_buf *linebuf, jit_op *op)
{
 char *str = (char *)op->arg[0];
 jit_internal_ob_append(linebuf, "// ");
 for (int i = 0; i < strlen(str); i++) {
  if ((str[i] == '\r') || (str[i] == '\n')) jit_internal_ob_append(linebuf, "\n// ");
  else jit_internal_ob_printf(linebuf, "%c", str[i]);
 }
}
int jit_internal_print_op(FILE *f, struct jit_disasm * disasm, struct jit_op *op, jit_tree *labels, int verbosity)
{
 struct output_buf *linebuf = jit_internal_ob_new();
 if ((((jit_opcode) (op->code & 0xfff8)) == JIT_LABEL) || (((jit_opcode) (op->code & 0xfff8)) == JIT_PATCH)) {
  ...
  if (lab) {
   jit_internal_ob_printf(linebuf, disasm->label_template, (((long)lab->value) < 0 ? - ((long)lab->value) : (long)lab->value));
   jit_internal_ob_printf(linebuf, ":");
  }
  goto print;
 }
 ...
 if (((jit_opcode) (op->code & 0xfff8)) == JIT_TRACE) {
  jit_internal_ob_append(linebuf, disasm->indent_template);
  jit_internal_ob_append(linebuf, ".trace");
  goto print;
 }
 char * op_name = jit_get_op_name(op);
 ...
 if (op_name[0] == '.') {
  switch (((jit_opcode) (op->code & 0xfff8))) {
   case JIT_DATA_BYTE:
   case JIT_CODE_ALIGN:
    jit_internal_ob_printf(linebuf, "%s ", op_name);
    jit_internal_ob_pad(linebuf, 13);
    jit_internal_ob_printf(linebuf, disasm->generic_value_template, op->arg[0]);
    goto print;
   case JIT_DATA_BYTES:
    jit_internal_ob_printf(linebuf, "%s ", op_name);
    jit_internal_ob_pad(linebuf, 13);
    for (int i = 0; i < op->arg[0]; i++) {
     jit_internal_ob_printf(linebuf, disasm->generic_value_template, ((unsigned char *)op->addendum)[i]);
     jit_internal_ob_printf(linebuf, " ");
    }
    goto print;
   case JIT_DATA_REF_CODE:
 ...
 jit_internal_ob_pad(linebuf, 12);
 if (op->arg_size == 1) jit_internal_ob_append(linebuf, " (byte)");
 if (op->arg_size == 2) jit_internal_ob_append(linebuf, " (word)");
 if (op->arg_size == 4) jit_internal_ob_append(linebuf, " (dword)");
 if (op->arg_size == 8) jit_internal_ob_append(linebuf, " (qword)");
 switch (((jit_opcode) (op->code & 0xfff8))) {
  ...
  case JIT_DECL_ARG:
   switch (op->arg[0]) {
    case JIT_SIGNED_NUM: jit_internal_ob_append(linebuf, " integer"); break;
    case JIT_UNSIGNED_NUM: jit_internal_ob_append(linebuf, " uns. integer"); break;
    case JIT_FLOAT_NUM: jit_internal_ob_append(linebuf, " float"); break;
    case JIT_PTR: jit_internal_ob_append(linebuf, " ptr"); break;
    default: assert(0);
   };
   jit_internal_ob_append(linebuf, ", ");
   jit_internal_print_arg(disasm, linebuf, op, 2);
...
int jit_internal_print_op_compilable(struct jit_disasm *disasm, struct jit_op * op, jit_tree * labels)
{
 ...
 if (lab && ((long) lab->value > 0)) {
  jit_internal_ob_printf(linebuf, "// ");
  jit_internal_ob_printf(linebuf, disasm->label_template, (long)lab->value);
  jit_internal_ob_printf(linebuf, ":\n");
 }
 if (((jit_opcode) (op->code & 0xfff8)) == JIT_COMMENT) {
  jit_internal_print_comment(linebuf, op);
 ...
 || (((jit_opcode) (op->code & 0xfff8)) == JIT_DATA_REF_CODE) || (((jit_opcode) (op->code & 0xfff8)) == JIT_DATA_REF_DATA))
  jit_internal_ob_printf(linebuf, "jit_op * op_%li = ", (((unsigned long) (op)) >> 4));
 switch (((jit_opcode) (op->code & 0xfff8))) {
  case JIT_LABEL: {
   jit_internal_ob_printf(linebuf, "jit_label * ");
   jit_tree * lab = jit_tree_search(labels, (long)op->arg[0]);
   if (lab) jit_internal_ob_printf(linebuf, disasm->label_template, (long) lab->value);
   jit_internal_ob_printf(linebuf, " = jit_get_label(p");
   goto print;
  }
  case JIT_PATCH:
   jit_internal_ob_printf(linebuf, "jit_patch  (p, op_%li", (((unsigned long) (op->arg[0])) >> 4));
   goto print;
  case JIT_DATA_BYTE:
   jit_internal_ob_printf(linebuf, "jit_data_byte(p, ");
   jit_internal_ob_printf(linebuf, disasm->generic_value_template, op->arg[0]);
   goto print;
  case JIT_DATA_BYTES:
   for (int i = 0; i < op->arg[0]; i++) {
    jit_internal_ob_printf(linebuf, "jit_data_byte(p, ");
    jit_internal_ob_printf(linebuf, disasm->generic_value_template, ((unsigned char *)op->addendum) [i]);
    if (i < op->arg[0] - 1) jit_internal_ob_printf(linebuf, ");\n");
   }
   goto print;
  case JIT_REF_CODE:
  case JIT_REF_DATA:
   jit_internal_ob_printf(linebuf, "jit_%s(p, ", jit_get_op_name(op));
   jit_internal_print_arg(disasm, linebuf, op, 1);
   jit_internal_ob_append(linebuf, ", ");
   jit_internal_print_addr(disasm, linebuf, labels, op, 1);
   goto print;
  case JIT_DATA_REF_CODE:
  case JIT_DATA_REF_DATA:
   jit_internal_ob_printf(linebuf, "jit_data_%s(p, ", jit_get_op_name(op) + 1);
   jit_internal_print_addr(disasm, linebuf, labels, op, 0);
   goto print;
  case JIT_CODE_ALIGN:
   jit_internal_ob_printf(linebuf, "jit_code_align  (p, ");
   jit_internal_ob_printf(linebuf, disasm->generic_value_template, op->arg[0]);
   goto print;
  case JIT_PREPARE:
   jit_internal_ob_printf(linebuf, "jit_prepare(p");
   goto print;
  default:
   break;
 }
 if (jit_get_op_name(op)[0] == '.') goto direct_print;
 jit_internal_ob_append(linebuf, "jit_");
 jit_internal_print_full_op_name(linebuf, op);
 jit_internal_ob_pad(linebuf, 15);
 jit_internal_ob_append(linebuf, "(p,");
 switch (((jit_opcode) (op->code & 0xfff8))) {
  case JIT_MSG:
   jit_internal_print_str(linebuf, (char *)op->arg[0]);
   if (!(op->code & 0x02)) {
  ...
  case JIT_DECL_ARG:
   switch (op->arg[0]) {
    case JIT_SIGNED_NUM: jit_internal_ob_append(linebuf, "JIT_SIGNED_NUM"); break;
    case JIT_UNSIGNED_NUM: jit_internal_ob_append(linebuf, "JIT_UNSIGNED_NUM"); break;
    case JIT_FLOAT_NUM: jit_internal_ob_append(linebuf, "JIT_FLOAT_NUM"); break;
    case JIT_PTR: jit_internal_ob_append(linebuf, "JIT_PTR"); break;
    default: assert(0);
   };
   jit_internal_ob_append(linebuf, ", ");
   jit_internal_print_arg(disasm, linebuf, op, 2);
 ...
 if (op->arg_size) jit_internal_ob_printf(linebuf, ", %i", op->arg_size);
print:
 jit_internal_ob_append(linebuf, ");");
direct_print:
